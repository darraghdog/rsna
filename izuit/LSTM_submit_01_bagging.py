...
for LSTM_UNITS in ['512']:
    for FOLD,START,BAG in zip([0],[0], [7]):
        #FOLD=1'lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
        fname = 'lstm{}deep_{}_emb_sz480_fold{}_epoch{}.csv.gz'
...
#         sublstm = pd.read_csv('~/Downloads/sub_lstm_emb_sz256_wt256_fold0_gepoch235.csv.gz', index_col= 'ID')
        ylstmsub = magicF((sum(lstmlssub)/len(lstmlssub)), 1.1).clip(0.00001, 0.99999)
        ylstmval = magicF((sum(lstmlsval)/len(lstmlsval)), 1.1).clip(0.00001, 0.99999)
...
ylstmsub = magicF(ylstmsub, 1.1).clip(0.00001, 0.99999)
...
ylstmsub.to_csv(os.path.join(results_path, './sub/sub_lstmCNN_emb_resnext101v12_sz480_LU_512.csv.gz'), \
            compression = 'gzip')
