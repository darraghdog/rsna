...
try:
    ...
    del([trnembT,valembT,tstembT, trnembP, valembP, tstembP])
    gc.collect()
except:
...
#             scaled_loss.backward()
    ...
    yvalout.to_csv(os.path.join(path_emb, '04_CNNlstm{}deep_val_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
    ...
    ytstout.to_csv(os.path.join(path_emb, '04_CNNlstm{}deep_sub_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
            index = False, compression = 'gzip')
    
    logger.info('Output model...')
    output_model_file = '/data/dmitry/RSNA/weights/resnext101v12/04_model_CNNlstm{}deep_{}.bin'.format(LSTM_UNITS, embnm)
