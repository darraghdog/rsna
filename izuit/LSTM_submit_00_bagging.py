...
def makeSub(ypred, imgs):
    ...
    return subdf
results_path = '/data/dmitry/RSNA/EMB/seresnext50v03'
# Load lstms
...
for LSTM_UNITS in ['1024']:
    for FOLD,START,BAG in zip([0,1,2],[0,0,1], [4,4,3]):
        #FOLD=1'lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
        fname = 'lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
...
#         sublstm = pd.read_csv('~/Downloads/sub_lstm_emb_sz256_wt256_fold0_gepoch235.csv.gz', index_col= 'ID')
        ...
        ylstmval = (sum(lstmlsval)/len(lstmlsval)).clip(0.00001, 0.99999)#magicF((sum(lstmlsval)/len(lstmlsval)), .95).clip(0.00001, 0.99999)
#         ylstmval["Label"] = magicF(gmean(np.hstack([x.values for x in lstmlsval]), axis = 1), 1.3).clip(0.00001, 0.99999)
...
fname = 'lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
for LSTM_UNITS in ['1024']:
    for FOLD,START,BAG in zip([0,1,2],[0]*3, [3]*3):
        lstmlssub += [pd.read_csv(os.path.join(results_path, \
                                         fname.format(LSTM_UNITS, 'sub', FOLD, i)), index_col= 'ID') for i in range(START,BAG)]
...
ylstmsub.to_csv(os.path.join(results_path, './sub/sub_lstmCNN_emb_seresnextv3_sz448_fold012_gepoch0123_LU_1024.csv.gz'), \
            compression = 'gzip')
