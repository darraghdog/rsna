...
path_emb='/data/dmitry/RSNA/EMB/resnext101v12/'#os.path.join('/data/sdsml_prod/projects/data/ldc/rsna/', options.workpath)
...
trn2mdf = pd.read_csv('/data/dmitry/RSNA/stage2/train_metadata2.csv')
tst2mdf = pd.read_csv('/data/dmitry/RSNA/stage2/test_metadata2.csv')
...
trnmdf[poscols] = pd.DataFrame(trnmdf['ImagePositionPatient']\
              .apply(lambda x: list(map(float, ast.literal_eval(x)))).tolist())
trn2mdf[poscols] = pd.DataFrame(trn2mdf['ImagePositionPatient']\
              .apply(lambda x: list(map(float, ast.literal_eval(x)))).tolist())
...
trnmdf = trnmdf.sort_values(['PatientID']+poscols)\
                [['PatientID', 'SOPInstanceUID']+poscols].reset_index(drop=True)
trn2mdf = trn2mdf.sort_values(['PatientID']+poscols)\
                [['PatientID', 'SOPInstanceUID']+poscols].reset_index(drop=True)

tstmdf = tstmdf.sort_values(['PatientID']+poscols)\
                [['PatientID', 'SOPInstanceUID']+poscols].reset_index(drop=True)
tst2mdf = tst2mdf.sort_values(['PatientID']+poscols)\
                [['PatientID', 'SOPInstanceUID']+poscols].reset_index(drop=True)

trnmdf['seq'] = (trnmdf.groupby(['PatientID']).cumcount() + 1)
trn2mdf['seq'] = (trn2mdf.groupby(['PatientID']).cumcount() + 1)
tstmdf['seq'] = (tstmdf.groupby(['PatientID']).cumcount() + 1)
tst2mdf['seq'] = (tst2mdf.groupby(['PatientID']).cumcount() + 1)
...
trnmdf.columns = trn2mdf.columns = tstmdf.columns  = tst2mdf.columns = ['PatientID', 'Image', 'seq']
...
tst2df = loadobj(os.path.join(path_emb, '/data/dmitry/RSNA/stage2/stg2tst/loader_tst2_size{}_fold{}_ep{}'.format(SIZE, fold, GLOBALEPOCH))).dataset.data
...
'''
# Add stg1 test labels to train
trn2df = pd.read_csv( '/data/dmitry/RSNA/stage2/train2.csv')
tst1df = trn2df.set_index('Image').loc[tstdf.Image].reset_index()
tst1df['fold'] = 5
trndf = pd.concat([trndf, tst1df], 0)
...
trndf = trndf.merge(trn2mdf.drop('PatientID', 1), on = 'Image')
...
embnm='emb_sz480_fold{}_epoch{}'.format(fold, GLOBALEPOCH)
'''
if LOADCSV:
...
tst2emb = np.load(os.path.join('/data/dmitry/RSNA/stage2/stg2tst/emb_tst2_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
try:
    trnembT = np.load(os.path.join(path_emb, 'embT_trn_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    valembT = np.load(os.path.join(path_emb, 'embT_val_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    tstembT = np.load(os.path.join(path_emb, 'embT_tst_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    tst2embT = np.load(os.path.join('/data/dmitry/RSNA/stage2/stg2tst/embT_tst2_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    
    trnembP = np.load(os.path.join(path_emb, 'embP_trn_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    valembP = np.load(os.path.join(path_emb, 'embP_val_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    tstembP = np.load(os.path.join(path_emb, 'embP_tst_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    tst2embP = np.load(os.path.join('/data/dmitry/RSNA/stage2/stg2tst/embP_tst2_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
    
    trnemb = (trnemb + trnembT + trnembP) / 3.
    valemb = (valemb + valembT + valembP) / 3.
    tstemb = (tstemb + tstembT + tstembP) / 3.
    tst2emb = (tst2emb+tst2embT+tst2embP) / 3.
    
    del([trnembT,valembT,tstembT, trnembP, valembP, tstembP, tst2embT,tst2embP])
    gc.collect()
except:
    logger.info('Failed to load T/P files')
...
logger.info('Tst2 shape {} {}'.format(*tst2emb.shape))

trnemb = np.concatenate([trnemb, tstemb],0)
...
class NeuralNet(nn.Module):
    def __init__(self, embed_size=int(trnemb.shape[-1]), LSTM_UNITS=64, DO = 0.3):
...
#         self.linear_1 = nn.Linear(LSTM_UNITS*2, n_classes-1)
#         self.linear_2 = nn.Linear(n_classes-1, 1)
...
# #         print("h_embedding",h_embedding.shape)
        ...
        h_embedding = self.embedding_dropout(h_embedding)#F.tanh(self.embedding_dropout(h_embedding))#.transpose(1,2)
#         print("h_embedding",h_embedding.shape)
...
#         output_1 = self.linear_1(hidden)
#         output_2 = self.linear_2(F.sigmoid(output_1))
#         output = torch.cat([output_1,output_2], 2)
...
#             scaled_loss.backward()
    ...
    yvalout.to_csv(os.path.join(path_emb, '05s2_CNNlstm{}deep_val_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
    ...
    ypred, imgtst = predict(tst2loader)
    ypredtstls.append(ypred)
    ytstpred = sum(ypredtstls[-nbags:])/len(ypredtstls[-nbags:])
    ytstout = makeSub(ytstpred, imgtst)
    ytstout.to_csv(os.path.join(path_emb, '05s2_CNNlstm{}deep_sub_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
            index = False, compression = 'gzip')
    
    logger.info('Output model...')
    output_model_file = '/data/dmitry/RSNA/weights/resnext101v12/05s2_model_CNNlstm{}deep_{}.bin'.format(LSTM_UNITS, embnm)
