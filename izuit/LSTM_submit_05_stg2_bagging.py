...
for LSTM_UNITS in ['2048']:
    for FOLD,START,BAG in zip([0,1,2],[0,0,0], [5,5,5]):
        #FOLD=1'lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
        fname = '05s2_CNNlstm{}deep_{}_emb_sz480_fold{}_epoch{}.csv.gz'
...
#         ylstmsub = magicF((sum(lstmlssub)/len(lstmlssub)), .9).clip(0.00001, 0.99999)
#         ylstmval = magicF((sum(lstmlsval)/len(lstmlsval)), .9).clip(0.00001, 0.99999)
        ylstmsub = (sum(lstmlssub)/len(lstmlssub)).clip(0.00001, 0.99999)
        ylstmval = (sum(lstmlsval)/len(lstmlsval)).clip(0.00001, 0.99999)
#         ylstmval["Label"] = magicF(gmean(np.hstack([x.values for x in lstmlsval]), axis = 1), 1.2).clip(0.00001, 0.99999)
...
ylstmsub.to_csv(os.path.join(results_path, './sub/submit_05s2_sub_lstmCNN_emb_resnext101v12_sz480_LU_2048.csv.gz'), \
            compression = 'gzip')
