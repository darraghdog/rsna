...
train1 = pd.read_csv(os.path.join(path_data, 'train.csv.gz'))
test1 = pd.read_csv(os.path.join(path_data, 'test.csv.gz'))

train = pd.read_csv(os.path.join(path_data, 'train2.csv.gz'))
test = pd.read_csv(os.path.join(path_data, 'test2.csv.gz'))
...
train1 = train1.set_index('Image').loc[png].reset_index()

# get fold
#valdf = train[train['fold']==fold].reset_index(drop=True)
#trndf = train[train['fold']!=fold].reset_index(drop=True)
foldimgs = train1[train1['fold']==fold]['Image'].tolist()
logger.info('Foldimgs shape {}'.format(len(foldimgs)))
logger.info('Train full shape {} {}'.format(*train.shape))

valdf = train[train['Image'].isin(foldimgs)].reset_index(drop=True)
trndf = train[~train['Image'].isin(foldimgs)].reset_index(drop=True)
