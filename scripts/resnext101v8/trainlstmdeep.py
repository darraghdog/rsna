...
if fold!=5: valdf = loadobj(os.path.join(path_emb, 'loader_val_size{}_fold{}_ep{}'.format(SIZE, fold, GLOBALEPOCH))).dataset.data
...
if fold!=5: valdf['embidx'] = range(valdf.shape[0])
tstdf['embidx'] = range(tstdf.shape[0])
trndf = trndf.merge(trnmdf.drop('PatientID', 1), on = 'Image')
if fold!=5:valdf = valdf.merge(trnmdf.drop('PatientID', 1), on = 'Image')
...
if fold!=5 :valemb = np.load(os.path.join(path_emb, 'emb_val_size{}_fold{}_ep{}.npz'.format(SIZE, fold, GLOBALEPOCH)))['arr_0']
...
if fold!=5: logger.info('Val shape {} {}'.format(*valemb.shape))
...
if fold !=5: valdataset = IntracranialDataset(valdf, valemb, labels=False)
...
if fold!=5 : valloader = DataLoader(valdataset, batch_size=batch_size*4, shuffle=False, num_workers=8, collate_fn=collatefn)
