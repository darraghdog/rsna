...
class NeuralNet(nn.Module):
    def __init__(self, embed_size=trnemb.shape[-1], LSTM_UNITS=64, DO = 0.3):
        ...
        self.lstm3 = nn.LSTM(embed_size*2, LSTM_UNITS, bidirectional=True, batch_first=True)
        ...
        self.linear = nn.Linear(LSTM_UNITS*4, n_classes)
    ...
    def forward(self, x, lengths=None):
        ...
        h_lstm1, _ = self.lstm1(h_embedding[:, :, :self.embsize])
        h_lstm2, _ = self.lstm2(h_lstm1)
        
        h_lstm3, _ = self.lstm3(h_embedding[:, :, self.embsize:])
        ...
        hidden1 = h_lstm1 + h_lstm2 + h_conc_linear1 + h_conc_linear2
        hidden3 = h_lstm3 + h_lstm4 + h_conc_linear3 + h_conc_linear4
        #print(hidden1.shape)
        #print(hidden3.shape)
        hidden  = torch.cat(( hidden1, hidden3 ), -1)
