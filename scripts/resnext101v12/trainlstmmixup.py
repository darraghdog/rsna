...
parser.add_option('-t', '--mixup_proba', action="store", dest="mixup_proba", help="Mixup proba", default="1.0")
parser.add_option('-u', '--beta', action="store", dest="beta", help="Mixup beta", default="1.0")
...
MIXUPPROBA=float(options.mixup_proba)
...
class IntracranialDataset(Dataset):
    def __init__(self, df, mat, labels=label_cols, train=False ):
        self.data = df
        self.mat = mat
        self.train = train
        self.labels = labels
        # Sort patients randomly keeping batchsize the same
        vc = df.SliceID.value_counts(ascending=True)  # df.SliceID.unique()
        ...
        # self.patients = df.SliceID.value_counts(ascending=True).index.tolist() # df.SliceID.unique()
    ...
    def __getitem__(self, idx):
        ...
        #if (random.randint(0,1)==0) and (self.train) :
        ...
        #patdeltalag2  = patdeltalag.copy()
        #patdeltalead2  = patdeltalead.copy()
        #patdeltalag2 [2:] += patemb[2:]-patemb[:-2]
        #patdeltalead2[:-2] += patemb[:-2]-patemb[2:]
...
trnloader = DataLoader(trndataset, batch_size=batch_size, shuffle=False, num_workers=8, collate_fn=collatefn)
...
for epoch in range(EPOCHS):
    ...
    model.train()  
    trndataset = IntracranialDataset(trndf, trnemb, labels=True, train = True)
    trnloader = DataLoader(trndataset, batch_size=batch_size, shuffle=False, num_workers=8, collate_fn=collatefn)
    for step, batch in enumerate(trnloader):
        ...
        if BETA > 0 and r < MIXUPPROBA:
            ...
            # Get loss
            loss = lam * (criterion(logits_a, target_a) + (1. - lam) * criterion(logits_b, target_b))
    ...
    def accKBag(yact, ypredls, K=10):
        yvalpred = sum(ypredls[-K:])/len(ypredls[-K:])
        yvalout = makeSub(yvalpred, imgval)
        weights = ([1, 1, 1, 1, 1, 2] * ypredls[-1].shape[0])
        return log_loss(yact['Label'].values, yvalout['Label'].values.clip(.00001,.99999) , sample_weight = weights)

    yvalpred = sum(ypredls[-nbags:])/len(ypredls[-nbags:])
    yvalout = makeSub(yvalpred, imgval)
    yvalp = makeSub(ypred, imgval)

    if epoch==EPOCHS-1: yvalout.to_csv('lstmv04/lstm{}{}{}delta_val_{}.csv.gz'.format(TTAHFLIP, TTATRANSPOSE, LSTM_UNITS, embnm), \
    ...
    logger.info('Epoch {} val logloss {:.5f} bagged {:.5f} Acc5Bag {:.5f} Acc10Bag {:.5f} Acc15Bag {:.5f}'.format(epoch, valloss, vallossavg, accKBag(yact, ypredls, K=5), accKBag(yact, ypredls, K=10), accKBag(yact, ypredls, K=15)))
    ...
    #        index = False, compression = 'gzip')
    if epoch==EPOCHS-1: ytstout.to_csv('lstmv04/lstm{}{}{}delta_sub_{}.csv.gz'.format(TTAHFLIP, TTATRANSPOSE, LSTM_UNITS, embnm), \
            index = False, compression = 'gzip')
