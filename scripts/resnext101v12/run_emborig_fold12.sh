...
do
    bsub  -q lowpriority -gpu "num=$N_GPU:mode=exclusive_process" -app gpu -n =$N_GPU  -env LSB_CONTAINER_IMAGE=darraghdog/kaggle:apex_build \
            ...
            --logmsg Rsna-lb-$SIZE-fp16 --start 1 --epochs 6 --fold $FOLD  --lr 0.00002 --batchsize 48  --workpath scripts/$WDIR  \
            --hflip F --infer EMB --imgpath data/mount/512X512X6/ --size $SIZE --weightsname weights/model_512_resnext101$FOLD.bin"
done
