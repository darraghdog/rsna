...
class IntracranialDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        patlag = patdf['ImagePos3_lag'].values[:, np.newaxis]
        ...
        patemb = np.concatenate((patemb, patdeltalag, patdeltalead, patlag), -1)
...
tstmdf['ImagePos3_lag'] = (tstmdf['ImagePos3']-tstmdf['ImagePos3'].shift(1))
tstmdf['ImagePos3_lag'][tstmdf['SliceID']!=tstmdf['SliceID'].shift(1)] = 0
...
class NeuralNet(nn.Module):
    def __init__(self, embed_size= 1 + trnemb.shape[-1]*3, LSTM_UNITS=64, DO = 0.3):
...
for epoch in range(EPOCHS):
    ...
    if epoch==EPOCHS-1: yvalout.to_csv('lstmv06/lstm{}{}{}delta_val_{}.csv.gz'.format(TTAHFLIP, TTATRANSPOSE, LSTM_UNITS, embnm), \
    ...
    #        index = False, compression = 'gzip')
    if epoch==EPOCHS-1: ytstout.to_csv('lstmv06/lstm{}{}{}delta_sub_{}.csv.gz'.format(TTAHFLIP, TTATRANSPOSE, LSTM_UNITS, embnm), \
            index = False, compression = 'gzip')
