...
for epoch in range(n_epochs):
    ...
    model.eval()
    valnpls = []
    for aug in [transform_test, transform_test1]:
        valls = []
        valdataset = IntracranialDataset(valdf, path=dir_train_img, \
                                         transform=aug, labels=False)
        valloader = DataLoader(valdataset, batch_size=batch_size*4, \
        ...
        valnpls.append(np.concatenate(valls, 0))
    weights = ([1, 1, 1, 1, 1, 2] * valdf.shape[0])
    yact = valdf[label_cols].values.flatten()
    ypred = (sum(valnpls)/len(valnpls)).flatten()
    ...
    valpreddf.to_csv('val_pred_{}_fold{}_epoch{}_samp{}_tta.csv.gz'.format(SIZE, fold, epoch, RANDSAMPLE), compression='gzip', index = False)
    if INFER == 'TST':
        tstpreddfls = []
        for aug in [transform_test, transform_test1]:
            tstls = []
            tstdataset = IntracranialDataset(test, path=dir_test_img, \
                                             transform=aug, labels=False)
            tstloader = DataLoader(tstdataset, batch_size=batch_size*4, \
                                   shuffle=False, num_workers=num_workers)
            ...
            tstpreddfls.append(np.concatenate(tstls, 0))
        tstpreddf = pd.DataFrame(sum(tstpreddfls)/len(tstpreddfls), columns = label_cols)
        test.to_csv('tst_act_fold.csv.gz', compression='gzip', index = False)
        tstpreddf.to_csv('tst_pred_{}_fold{}_epoch{}_samp{}_tta.csv.gz'.format(SIZE, fold, epoch, RANDSAMPLE), compression='gzip', index = False)
