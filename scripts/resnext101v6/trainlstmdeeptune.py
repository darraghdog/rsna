...
parser.add_option('-x', '--decay', action="store", dest="decay", help="Learning rate decay", default="0.8")
parser.add_option('-k', '--augmentation', action="store", dest="augmentation", help="Randomly Flip Sequence", default="F")
...
AUGMENTATION= options.augmentation=='T'
...
class IntracranialDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if AUGMENTATION and (np.random.randint(2)==1):
            patdf = self.data.loc[patidx].sort_values('seq', ascending = False)
        else:
            patdf = self.data.loc[patidx].sort_values('seq')
        if AUGMENTATION  and (np.random.randint(5)<2):
            try:
                seqlen = np.random.randint(min(10, patdf.shape[0]-10))
                start = np.random.randint(seqlen)
                patdf = patdf[(start) : -(seqlen-start)]
            except:
                1
...
lambda2 = lambda epoch: DECAY ** epoch
...
for epoch in range(EPOCHS):
    ...
    yvalep = makeSub(ypredls[-1], imgval)

    yvalout.to_csv(os.path.join(path_emb, 'lstmdeep{}_val_tune_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
    ...
    vallossep  = log_loss(yact['Label'].values, yvalep ['Label'].values.clip(0.0005, 0.9995), sample_weight = weights)
    def vlossfn(N): return log_loss(yact['Label'].values, makeSub( sum(ypredls[-N:])/N, imgval)['Label'].values.clip(0.0005, 0.9995), sample_weight = weights)

    logger.info('Epoch {} val logloss {:.5f} bag4 {:.5f} bag6 {:.5f} bag9 {:.5f} bag15 {:.5f}'\
               .format(epoch, vallossep, vlossfn(4), vlossfn(6), vlossfn(9), vlossfn(15)))
    ...
    ytstout.to_csv(os.path.join(path_emb, 'lstmdeep{}_sub_tune_{}.csv.gz'.format(LSTM_UNITS, embnm)), \
            index = False, compression = 'gzip')
    
    logger.info('Output model...')
    output_model_file = 'weights/model_lstmunits{}_tune_{}.bin'.format(LSTM_UNITS, embnm)
