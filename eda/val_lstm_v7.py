...
for LSTM_UNITS in ['', '1024']:
    for FOLD,BAG in zip([0,1,2], [7,5,7]):
        ...
        ylstmsub = sum(lstmlssub)/len(lstmlssub) # hmean([(l+1.).values for l in lstmlssub], 0)-1 # 
        ylstmval = sum(lstmlsval)/len(lstmlsval) # hmean([(l+1.).values for l in lstmlsval], 0)-1 # 
        ylstmsub = ylstmsub.clip(0.00001, 0.99999)
        ylstmval = ylstmval.clip(0.00001, 0.99999)
        #ylstmsub = pd.DataFrame({'Label':ylstmsub.flatten()}, index = lstmlssub[0].index)
        #ylstmval = pd.DataFrame({'Label':ylstmval.flatten()}, index = lstmlsval[0].index)
