...
for LSTM_UNITS in ['1024']:
    for FOLD,START,BAG in zip([0, 1, 2],[0, 0, 0], [5, 5, 6]):
        #FOLD=1
        fname = 'seq/v11/lstm/lstm{}deep_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz'
...
ylstmsub.to_csv(os.path.join(path, \
            '../sub/sub_lstmdeep_emb_resnext101v11_sz384_fold012_gepoch01234_LU_1024.csv.gz'), \
            compression = 'gzip')

    '''
subtop1 = pd.read_csv(os.path.join(path, 
                        '../sub/sub_lstmdeep_emb_resnextv6_sz384_fold012_gepoch123456_bag6_LU_256_1024.csv.gz'), index_col= 'ID')
subtop2 = pd.read_csv(os.path.join(path, 
                        '../sub/sub_lstmdeep_emb_seresnextv3_sz448_fold0123_gepoch0123_LU_1024.csv.gz'), index_col= 'ID')


print(pd.concat([subtop1, ylstmsub], 1).corr())
print(pd.concat([subtop2, ylstmsub], 1).corr())
print(pd.concat([subtop1, subtop2], 1).corr())

subbag = subtop1.copy()
subbag['Label'] = ( (subtop.loc[subbag.index]['Label']+subtop.loc[subbag.index]['Label'])/2 \
                      + ylstmsub.loc[subbag.index]['Label'] ) / 2
subbag.to_csv(os.path.join(path, '../sub/sub_bag_lstm_resnextv6_seresnextv3_resnextv11.csv.gz'), \
            compression = 'gzip')
'''
