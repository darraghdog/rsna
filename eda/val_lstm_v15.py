...
for LSTM_UNITS in ['2048']:
    for FOLD,START,BAG,TTA in zip([0,1,2],[0]*3,[5]*3,['TP']*3):
        fnamels =['seq/v12/lstmv07/lstm{}{}delta_epoch'+ str(epoch) +'_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz']
        #fnamels = ['seq/v12/lstmv06/lstm{}{}delta_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz']
        #fnamels = ['seq/v13/lstmv07/lstm{}{}delta_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz']
        lstmlssub = []
        lstmlsval = []
        ...
        if FOLD == 0: 
            TTA='T'
            fnamels =['seq/v13/lstmv07/lstm{}{}delta_epoch'+ str(epoch) +'_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz']
            for fname in fnamels:
                lstmlssub += [pd.read_csv(os.path.join(path_data, \
                                                 fname.format(TTA, LSTM_UNITS, 'sub', FOLD, i)), index_col= 'ID') for i in range(START,BAG)]
                lstmlsval += [pd.read_csv(os.path.join(path_data, \
...
'''
baseline
Fold 0 Epoch 5 Units 2048 bagged val logloss 0.05705
Fold 1 Epoch 5 Units 2048 bagged val logloss 0.05866
Fold 2 Epoch 5 Units 2048 bagged val logloss 0.05645
New results
Fold 0 Epoch 5 Units 2048 bagged val logloss 0.05646
Fold 1 Epoch 5 Units 2048 bagged val logloss 0.05866
Fold 2 Epoch 5 Units 2048 bagged val logloss 0.05645
'''
...
fnamels =['seq/v13/lstmv07/lstm{}{}delta_epoch'+ str(epoch) +'_{}_emb_sz256_wt256_fold{}_epoch{}.csv.gz']
for LSTM_UNITS in ['2048']:
    for FOLD,START,BAG,TTA in zip([0],[0],[5],['T']):
        for fname in fnamels:
            lstmlssub += [pd.read_csv(os.path.join(path_data, \
                fname.format(TTA, LSTM_UNITS, 'sub', FOLD, i)), index_col= 'ID') for i in range(START,BAG)]
...
ylstmsub.to_csv(os.path.join(path, \
            '../sub/sub_lstmdelta_emb_resnext101v12_fold012__resnext101v13_fold0_gepoch01234_LU_2048.csv.gz'), \
            compression = 'gzip')