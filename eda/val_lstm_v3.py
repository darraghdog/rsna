...
# Load lstms
fname = 'seq/v4/lstmdeep_{}_emb_sz256_wt256_fold0_epoch{}.csv.gz'
lstmlssub = [pd.read_csv(os.path.join(path_data, \
                                     fname.format('sub', i)), index_col= 'ID') for i in range(2,6)]
lstmlsval = [pd.read_csv(os.path.join(path_data, \
                                     fname.format('val', i)), index_col= 'ID') for i in range(2,6)]
...
ylstmsub.to_csv(os.path.join(path, '../sub/sub_lstmdeep_emb_resnextv4_sz256_wt256_fold0_gepoch2345_bag3.csv.gz'), \
            compression = 'gzip')
